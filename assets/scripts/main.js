...
function initializeServiceWorker() {
  ...
  // B1. TODO - Check if 'serviceWorker' is supported in the current browser
    ...
    window.addEventListener('load', async() => {
      ...
      if (registration) { console.log("Successfully registered service worker."); }
      // B5. TODO - In the event that the service worker registration fails, console
      //            log that it has failed.
      else { console.error(`Registration failed with ${error}`); }
  ...
  return new Promise((resolve, reject) => {
   ...
   RECIPE_URLS.forEach(async(url) => {
     ...
     try {
        ...
        //            "await" again
        const getJSON = await recipe.json();
        console.log(getJSON);
        // A8. TODO - Add the new recipe to the recipes array
        recipeArrayFetch.push(getJSON);
        if (recipeArrayFetch.length == RECIPE_URLS.length) {
          ...
          //            we have provided. Then, pass the recipes array <created by me> to the Promise's
          //            resolve() method.
          saveRecipesToStorage(recipeArrayFetch);
          resolve(recipeArrayFetch);
        }
      } catch (error) {
       // CATCH ***
